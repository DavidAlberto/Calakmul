...
q
pdf("Resultados/PlotDistribution.pdf", width=20, height=10)
q
dev.off()
svg("Resultados/PlotDistribution.svg", width=20, height=10)
q
dev.off()
pdf("Resultados/PlotDistribution1.pdf", width=20, height=10)
q + facet_wrap("SITE_ID")
dev.off()
svg("Resultados/PlotDistribution1.svg", width=20, height=10)
q + facet_wrap("SITE_ID")
dev.off()
```
...
```{r}
# Version 1 - Phylum
OnePercentageP <- unique(Phylum_R_DF$Phylum[Phylum_R_DF$Abundance > 1.0])
OnePercentageP
# Version 2 - Genus
OnePercentageG <- unique(Genus_R_DF$Phylum[Genus_R_DF$Abundance > 1.0])
OnePercentageG
```
...
for (i in OnePercentageP){
  ...
  print(dendro)
  pdf(paste0("Resultados/Dendogramas/PlotTree", i, "-P.pdf"), width = 20, height = 10)
  print(dendro)
  dev.off()
  svg(paste0("Resultados/Dendogramas/PlotTree", i, "-P.svg"), width = 20, height = 10)
  print(dendro)
  dev.off()
}
...
for (i in OnePercentageG){
  ...
  # i <- "Actinobacteriota"
  # Primero seleccionamos solo el Phylum
  Sub <- subset_taxa(Phylo_G, Phylum == i)
  # Filtramos por abundancia mayor a 5 reads
  Sub <- prune_samples(sample_sums(Sub) >= 5, Sub) 
  # Vemos resultados en un Dendograma
  tryCatch({
  ...
  print(dendro)
  pdf(paste0("Resultados/Dendogramas/PlotTree", i, "-G.pdf"), width = 20, height = 10)
  print(dendro)
  dev.off()
  svg(paste0("Resultados/Dendogramas/PlotTree", i, "-G.svg"), width = 20, height = 10)
  print(dendro)
  dev.off()
}
```
...
alpha_pd_P <- estimate_pd(Phylo_P)
# Combinamos la metadata con alpha.diversity
data_P <- cbind(sample_data(Phylo_P), alpha_pd_P) 
# Y calculamos un ANOVA
Phylo_P.anova <- aov(PD ~ SITE_ID, data_P) 
# install.packages("xtable")
Phylo_P.anova.table <- xtable(Phylo_P.anova)

# Version 2 - Genus
# Guardamos un dataframe con las medidas de diversidad alfa
alpha_pd_G <- estimate_pd(Phylo_G)
# Combinamos la metadata con alpha.diversity
data_G <- cbind(sample_data(Phylo_G), alpha_pd_G) 
# Y calculamos un ANOVA
Phylo_G.anova <- aov(PD ~ SITE_ID, data_G) 
# install.packages("xtable")
Phylo_G.anova.table <- xtable(Phylo_G.anova)
```
...
for (d in Distancia){
  ...
  beta <- plot_tsne_phyloseq(physeq = Phylo_P.2, tSNE, color = "SITE_ID") +
    ...
    scale_color_brewer(palette = "Set1") + 
    scale_fill_brewer(palette = "Set1")
  pdf(paste0("Resultados/BetaDiversidad/PlotBetatSNE-", d, "-P.pdf"), 
      width=20, height=10)
  print(beta)
  dev.off()
  svg(paste0("Resultados/BetaDiversidad/PlotBetatSNE-", d, "-P.svg"),
      width=20, height=10)
  print(beta)
}
...
for (d in Distancia){
  # Prueba 
  # d <- "bray"
  tSNE <- tsne_phyloseq(physeq = Phylo_G.2, distance= d, perplexity = 8, 
                        verbose=0, rng_seed = 3901)
  beta <- plot_tsne_phyloseq(physeq = Phylo_G.2, tSNE, color = "SITE_ID") +
    theme_classic() +
    geom_point(size = 5) +
    geom_text(mapping = aes(label = SITE_ID), size = 4, vjust = 2, hjust = 1) + 
    geom_vline(xintercept = 0) +
    ...
    scale_color_brewer(palette = "Set1") + 
    scale_fill_brewer(palette = "Set1")
  pdf(paste0("Resultados/BetaDiversidad/PlotBetatSNE-", d, "-G.pdf"), 
      width=20, height=10)
  print(beta)
  dev.off()
  svg(paste0("Resultados/BetaDiversidad/PlotBetatSNE-", d, "-G.svg"),
      width=20, height=10)
  print(beta)
}
```
...
# Version 2 - Genus
# head(Phylo_G@otu_table@.Data) # Aquí esta mal, esta al contrario
# Hacemos un nuevo objeto que tenga una transpuesta de Phylo_G@otu_table@.Data
...
# Absolutos
# Filtramos aquellos géneros que tienen una media mínima de 100 lecturas absolutas
psd.abs.100G <- filter_taxa(PhyloseqG, function(x) mean(x) > 100.0, TRUE)

# Relativos
psd.relG <- transform_sample_counts(PhyloseqG, function(x) x*100 / sum(x))
# Filtramos aquellos géneros que tiene al menos un 1 % de lecturas relativas 
psd.rel.1G <- filter_taxa(psd.relG, function(x) mean(x) > 1.0, TRUE)
```
