...
summary(psd5@tax_table@.Data== "") # Ya no hay ninguno
```
...
Abs.plot <- ggplot(data=Absolutos, aes(x=SITE_ID, y=Abundance, fill=Phylum)) + 
...
Rel.plot <- ggplot(data=Relativos, aes(x=SITE_ID, y=Abundance, fill=Phylum)) + 
...
Otra versión pero ahora en el eje X usamos la variable "ID" y en Y es "Abundance" y rellenamos por "Phylum".

```{r, include = TRUE, echo = TRUE, fig.pos = 'H', fig.dim = c(12,8), fig.align = "center", message=FALSE, fig.cap = 'Abundancia absoluta y relativa a nivel de *Phylum*, v2.'}
Abs.plot1 <- ggplot(data=Absolutos, aes(x=ID, y=Abundance, fill=Phylum)) + 
  geom_bar(aes(), stat="identity", position="stack") + 
  theme_classic() +
  labs(x="Samples", y="Absolute abundance",
       title = "Absolute abundance of analyzed samples") +
...
Rel.plot1 <- ggplot(data=Relativos, aes(x=ID, y=Abundance, fill=Phylum)) + 
  geom_bar(aes(), stat="identity", position="fill") + 
  theme_classic() +
  labs(x="Samples", y="Relative abundance",
       title = "Relative abundance of analyzed samples") +
...
pdf("Resultados/Abundancias/PlotRaw1.pdf", width=10, height=7)
Abs.plot1
dev.off()
svg("Resultados/Abundancias/PlotRaw1.svg", width=10, height=7)
Abs.plot1
dev.off()
pdf("Resultados/Abundancias/PlotRel1.pdf", width=10, height=7)
Rel.plot1
dev.off()
svg("Resultados/Abundancias/PlotRel1.svg", width=10, height=7)
Rel.plot1
dev.off()
pdf("Resultados/Abundancias/PlotRaw-Rel1.pdf", width=15, height=7)
Abs.plot1 | Rel.plot1
dev.off()
svg("Resultados/Abundancias/PlotRaw-Rel1.svg", width=15, height=7)
Abs.plot1 | Rel.plot1
dev.off()
```
...
```{r}
Relativos$Phylum <- as.character(Relativos$Phylum) # Primero lo convertimos a letra
Relativos$Phylum[Relativos$Abundance < 1.0] <- "Phylum < 1.0% abund." # Renombramos
unique(Relativos$Phylum)
```

Volvemos a graficar usando ese filtro de "Phylum < 1.0% abund."

```{r, include = TRUE, echo = TRUE, fig.pos = 'H', fig.dim = c(12,8), fig.align = "center", message=FALSE, fig.cap = 'Abundancia absoluta y relativa a nivel de *Phylum*. con Phylum < 1.0 % abundancia.'}
Rel.plot2 <- ggplot(data=Relativos, aes(x=SITE_ID, y=Abundance, fill=Phylum)) + 
  geom_bar(aes(), stat="identity", position="fill") + 
  theme_classic() +
  labs(x="Samples", y="Relative abundance",
       title = "Relative abundance of analyzed samples") +
...
```{r, include = TRUE, echo = TRUE, fig.pos = 'H', fig.dim = c(12,8), fig.align = "center", message=FALSE, fig.cap = 'Abundancia absoluta y relativa a nivel de *Phylum*. con Phylum < 1.0 % abundancia v2.'}
Rel.plot3 <- ggplot(data=Relativos, aes(x=ID, y=Abundance, fill=Phylum)) + 
  geom_bar(aes(), stat="identity", position="fill") + 
  theme_classic() +
  labs(x="Samples", y="Relative abundance",
       title = "Relative abundance of analyzed samples") +
...
pdf("Resultados/Abundancias/PlotRel3.pdf", width=10, height=7)
Rel.plot3
dev.off()
svg("Resultados/Abundancias/PlotRel3.svg", width=10, height=7)
Rel.plot3
dev.off()
pdf("Resultados/Abundancias/PlotRaw-Rel3.pdf", width=15, height=7)
Abs.plot1 | Rel.plot3
dev.off()
svg("Resultados/Abundancias/PlotRaw-Rel3.svg", width=15, height=7)
Abs.plot1 | Rel.plot3
dev.off()
```
...
for (i in OnePercentage){
  tryCatch({
    # Prueba 
    # i <- "Actinobacteriota"
    phy <- subset_taxa(psd5, Kingdom == "Bacteria") # Nos quedamos solo con Bacterias
    phy <- subset_taxa(psd5, Phylum == i) # Nos quedamos con el phylum de interés
    # Comprobamos
...
```{r, message = FALSE, warning = FALSE}
# Riqueza
Riqueza <- richness(psd5)
Riqueza
# Dominancia
Dominancia <- dominance(psd5, index = "all")
Dominancia
# Rareza
Rareza <- rarity(psd5, index = "all")
Rareza
# Cobertura
Cobertura <- coverage(psd5, threshold = 0.5)
Cobertura
# Desigualdad
Desigualdad <- inequality(psd5)
Desigualdad
# Uniformidad
Uniformidad <- evenness(psd5, "all")
Uniformidad
```
...
```{r}
names(Alpha2)
psd5.2.meta$Shannon <- Alpha2$diversity_shannon 
psd5.2.meta$InverseSimpson <- Alpha2$diversity_inverse_simpson
psd5.2.meta$Chao1 <- Alpha2$chao1
psd5.2.meta$Observed <- Alpha2$observed
psd5.2.meta$Fisher <- Alpha2$diversity_fisher
```
...
Area <- levels(as.factor(psd5.2.meta$SITE_ID))
...
for (i in Indices){
  # Prueba 
  # i <- "Observed"
  p <- ggviolin(psd5.2.meta, x = "SITE_ID", y = i, 
...
psd5.2@sam_data@.Data[[6]] <- as.factor(psd5.2@sam_data@.Data[[6]])
for (o in Ordenacion){
  # Prueba 
  # o <- "NMDS"
  for (d in Distancia){
    # Prueba 
    # d <- "bray"
    Ord <- ordinate(physeq = psd5.2, method = o, distance = d)
    Beta <- plot_ordination(physeq = psd5.2, ordination = Ord, 
...
for (d in Distancia){
  # Prueba 
  # d <- "bray"
  tSNE <- tsne_phyloseq(physeq = psd5.2, distance= d, perplexity = 8, 
                        verbose=0, rng_seed = 3901)
  beta <- plot_tsne_phyloseq(physeq = psd5.2, tSNE, color = "SITE_ID") +
    theme_classic() +
    geom_point(size = 5) +
    geom_text(mapping = aes(label = SITE_ID), size = 4, vjust = 2, hjust = 1) + 
    geom_vline(xintercept = 0) +
...
for (a in Area){
  ...
  phy <- subset_samples(psd5.2, SITE_ID == a)
...
top10 <- get_top_taxa(physeq_obj = psd5.2, n = 10, relative = T,
...
psd6 <- merge_phyloseq(otu_table(t(psd5@otu_table@.Data), taxa_are_rows = T), 
                       psd5@tax_table, 
                       psd5@sam_data, 
                       psd5@phy_tree, 
                       psd5@refseq)
head(psd6@otu_table@.Data) # Ya está como la queremos
summary(psd6@otu_table@.Data) # Resumen estadístico
```
...
psd.abs.100 <- filter_taxa(psd7, function(x) mean(x) > 100.0, TRUE)

# Relativos
psd.rel <- transform_sample_counts(psd7, function(x) x*100 / sum(x))
...
Genero <- tax_glom(psd5.2, taxrank = 'Genus')

# Absolutos
# Generamos el data.frame
V.Absolutos <- psmelt(Genero)
str(V.Absolutos)
...
# Generamos el data.frame
V.Relativos <- psmelt(Transfomacion)
str(V.Relativos)
...
```{r, include = TRUE, echo = TRUE, fig.pos = 'H', fig.dim = c(12,8), fig.align = "center", message=FALSE, fig.cap = 'Gráficos de abundancia a diferentes niveles taxonómicos.'}
...
for (t in Taxon){
  ...
  # t <- c("Family")
  # Absoluto
  plot.a <- ggplot(data = V.Absolutos, aes(x = V.Relativos[,t], y = Abundance, 
                                           fill = SITE_ID)) +
  geom_bar(aes(), stat = "identity", position = "stack") + 
  theme_classic() +
  ...
  dev.off()
  # Relativo
  plot.r <- ggplot(data = V.Relativos, aes(x = V.Relativos[,t], y = Abundance, 
                                           fill = SITE_ID)) + 
    geom_bar(aes(), stat = "identity", position = "fill") + 
    theme_classic() + 
