...
Abundancias a nivel de Phylum nuevamente para los deu abundancia mayor al 1 %
...
for (o in Ordenacion){
  ...
  for (d in Distancia){
    ...
    Beta <- plot_ordination(physeq = psd5.2, ordination = Ord, color = "SITE_ID", shape = "YEAR") +
      theme_classic() +
      geom_point(size = 5) +
      geom_text(mapping = aes(label = SITE_ID), size = 4, vjust = 2, hjust = 1) + 
      geom_vline(xintercept = 0) +
      geom_hline(yintercept = 0) +
      labs(title = paste("Beta diversity with", o, "-", d, "for the Calakmul samples rRNA 16s")) +
             theme(legend.position = "right", 
                   legend.text = element_text(size=12, face="bold"),
                   legend.title = element_text(size = 12, face = "bold"),
                   axis.text = element_text(color = "black", size = 12, face = "bold"),
